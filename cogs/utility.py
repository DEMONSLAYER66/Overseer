...
############################ PROMOTE #########################
    ...
    async def promote(self, ctx):
        ...
        if cooldown_data:
            ...
            cooldown_embed = discord.Embed(title=f"{ctx.guild.name}\nPromotion Cooldown", description=f"> Apologies good sir, it appears that my </{promote_app_command.name}:{promote_app_command.id}> directive is not ready to use at the moment...\n> Please try again in `{cooldown_time}`.\n> \n> *I apologize for the inconvenience.*", color=discord.Color.from_rgb(130, 130, 130))
    ...
    class ReadyOrNotView(discord.ui.View):
        ...
        def create_embed(self, ctx):
            ...
            if self.timed_out is True: #if timed out, change description
                desc = f"Good fellows of ***{self.ctx.guild.name}***,\n{self.ctx.author.mention} is no longer looking to play a game with other individuals."
            else:
        ...
        if autopurge_status == "Disabled":
            ...
            if guild_id != support_guild_id:
                ...
                if not distinguished_patron and autopurge_config_count > 5:
                    ...
                    autopurge_db[f"autopurge_config_{guild_id}"].delete_many(delete_key, limit=excess_count) #remove excess configurations
